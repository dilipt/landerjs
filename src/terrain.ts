import { Vector, TerrainData, Segment } from './types.js';

export class TerrainGenerator {
  generateSurface(width: number, height: number, segmentCount: number = 20): TerrainData {
    const points: Vector[] = [];
  ...
  private pointsToSegments(points: Vector[]): Segment[] {
  ...
  private createLandingPad(points: Vector[]): { startIdx: number; endIdx: number } {
  ...
  private calculateVariance(points: Vector[], startIdx: number, endIdx: number): number {
    const slopes: number[] = [];
